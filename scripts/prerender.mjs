// scripts/prerender.mjs  (laplandluxuryvillas)
//
...
//
// Dynamic villa + destination slugs are discovered from src/lib at run time so
// the route list never drifts from the app.
...
// ---- Discover dynamic slugs from source (never drift) ----------------------
async function discoverSlugsFrom(relFile) {
  try {
    const src = await readFile(join(ROOT, relFile), 'utf8')
...
async function main() {
  ...
  const villaSlugs = await discoverSlugsFrom(join('src', 'lib', 'villas.ts'))
  const destSlugs = await discoverSlugsFrom(join('src', 'lib', 'destinations.ts'))
  const ROUTES = [
    ...STATIC_ROUTES,
    ...villaSlugs.map((s) => `/villas/${s}`),
    ...destSlugs.map((s) => `/destinations/${s}`),
  ]
  console.log(
    `[prerender] routes: ${STATIC_ROUTES.length} static + ${villaSlugs.length} villas + ` +
      `${destSlugs.length} destinations = ${ROUTES.length} × ${LOCALE_PREFIXES.length} locales = ` +
