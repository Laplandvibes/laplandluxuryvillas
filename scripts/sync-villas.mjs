#!/usr/bin/env node
/**
 * sync-villas.mjs — pull REAL Google review data for every named property in
 * the canonical registry (`src/data/properties.ts`).
 *
 * Re-sync:
 *   cd laplandluxuryvillas-new && node scripts/sync-villas.mjs
 *
 ...
 *   src/data/generated/villas-from-maps.json   (gitted, keyed by registry key)
 *
 * It NEVER writes src/data/properties.ts or src/lib/villas.ts. Those are the
 * hand-maintained editorial layers and are merged with this snapshot at module
 * load, so re-running the sync can never clobber editorial work.
 *
 * 🔴 SCOPE: the cards on this site name ROOM TYPES ("Nellim Aurora Bubble"),
 * while Google rates the PROPERTY that contains the room. The registry keys are
 * therefore properties, not villas, and every rating rendered on the site
 * states that scope in visible copy. See the header of src/data/properties.ts.
 *
 * ── FAIL CLOSED ──────────────────────────────────────────────────────────────
 ...
 * exactly like a correct one. The network has shipped that class of bug before
 * (the laplandstays Ylläs page once presented a hotel at Iso-Syöte, 200 km
 * away), so a candidate is accepted only when ALL THREE gates pass:
 *
 ...
 */
const TARGETS = {
  kakslauttanen:   { queryCity: 'Saariselka, Lapland, Finland',  locality: ['saariselka', 'kakslauttanen', 'inari', 'ivalo'] },
  ...
  levinIglut:      { queryCity: 'Levi, Kittila, Lapland, Finland', locality: ['levi', 'sirkka', 'kittila'] },
 ...
 *
 * Inherited from laplandstays, where it caught a real wrong match rather than a
 * hypothetical one: chain siblings ("Lapland Hotels Saaga / Sirkantähti /
 * Ylläskaltio / …") share a 13-character prefix, so Dice bigram similarity
 * between two DIFFERENT hotels of one chain is structurally high. There, the
 * first sync run scored "Lapland Hotels Ylläs" against "Lapland Hotels
 * Äkäshotelli" at 0.79 and accepted it, pinning one hotel's reviews onto a card
 * naming another.
 *
 * No entry in THIS registry currently belongs to such a family, so the guard
 * binds on nothing today. It is kept because the collection is edited by hand
 * and Lapland's resort brands (Lapland Hotels, Wilderness Hotels, Santa's
 * Hotels) are exactly the kind of names an editor adds next.
 *
...
async function main() {
  ...
  for (const prop of registry) {
    ...
    if (!target) {
      unmatched.push({ key: prop.key, name: prop.name, reason: 'no TARGETS entry in sync-villas.mjs (add localities before syncing)' });
  ...
  const outPath = path.join(generatedDir, 'villas-from-maps.json');
  await fs.writeFile(
    outPath,
    JSON.stringify(
      {
        _README:
          'GENERATED by scripts/sync-villas.mjs from Google Places API (New) Text Search. ' +
  ...
  console.log(`+ wrote src/data/generated/villas-from-maps.json`);
}
