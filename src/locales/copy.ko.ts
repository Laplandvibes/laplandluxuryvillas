...
const ko: SiteCopy = {
  ...
  home: {
    philosophy: {
      ...
      items: [
        ...
        { title: '가격은 숙소에 있습니다. 저희에게가 아니라.', body: '공개 예약 페이지가 있는 빌라는 링크가 바로 그곳으로 연결되어 결정 전에 요금이 보입니다. 공개된 요금이 없으면 숙소가 직접 견적을 드립니다. 저희는 가격을 정하지 않고 예약도 받지 않습니다.' },
      ],
    },
    signature: {
      eyebrow: '시그니처 빌라',
  ...
  villasPage: {
    ...
    leadP1: '호텔 여덟 곳이 아니라 객실 여덟 개입니다. 여기 실린 항목은 모두 실재하는 숙소 안의 특정 객실 유형입니다. 카크슬라우타넨의 유리 지붕 캐빈, 아크틱 트리하우스의 돌출 스위트, 이나리 호수의 오로라 버블 같은 단위입니다. 실제로 예약하는 단위가 그것이므로, 우리도 그 단위로 싣습니다.',
  ...
  suitesPage: {
    ...
    inquiryTitle: '스위트 두 개 예약, 아니면 프라이빗 한 개 층?',
    inquiryBody: '이 가운데 몇몇 물건은 비수기 사이 주간에 여러 스위트로 이루어진 한 개 층을 단독 사용으로 엽니다. 날짜와 인원을 알려 주세요.',
  },
  destinationsPage: {
  ...
  midnightSunPage: {
    ...
    summerCollection: {
      ...
      lede: '겨울 컬렉션과 같은 건축 카테고리, 글래스 루프 캐빈, 호숫가 통나무 저택, 알파인 샬레, 숲속 빌라, 를 6월 말 백야의 빛 속에서 이야기의 후반부로 보여 드립니다.',
      cards: [
...
Object.assign(ko.suitesPage, {
  intro: { p1Pre: '라플란드의 "스위트"는 알아둘 만한 두 가지 건축적 접근을 포괄합니다. 첫째는 ', designerSuite: '디자이너 스위트', p1Mid: ', 핀란드의 건축 스튜디오(Studio Puisto 등)가 하나의 침실을 하나의 풍경에 맞춰 지은 것입니다. 둘째는 ', glassRoof: '글래스 루프 캐빈', p1Post: ', 천장이 헤드라인 디테일입니다. 두 가지 모두 아래에 정리되어 있습니다.' },
...
Object.assign(ko.midnightSunPage, {
  ...
  planning: { h2: '백야 여행 계획에 대한 짧은 메모.', p1: '이 기간은 짧고 빌라는 개별 소유이므로, 캘린더는 균등하기보다 기회적으로 채워집니다. 일찍 문의하시는 편이 좋습니다.', p2: '특정 6월 문의의 적기는 그 해 1~3월입니다. 그 이후의 문의도 보낼 가치가 있습니다, 출시가 발생하니까요, 그러나 고급 인벤토리는 종종 더 일찍 나갑니다.', quoting: '현재 2026 + 2027 여름 문의 접수 중' },
