...
const COPY: Record<Lang, { eyebrow: string; h2: string; lede: string; cta: string; from: string; priceNote: (p: string, v: string) => string }> = {
  ...
  es: { eyebrow: 'Reservable', h2: 'Experiencias privadas', lede: 'Cada tarjeta abre la página de reserva del propio operador. Eliges fecha y añades al carrito, no buscas.', cta: 'Ver disponibilidad', from: 'desde', priceNote: (p, v) => `Precios de GetYourGuide, leídos el ${p}. Páginas comprobadas el ${v}.` },
  ...
  nl: { eyebrow: 'Te boeken', h2: 'Privé-ervaringen', lede: 'Elke kaart opent de boekingspagina van de aanbieder zelf. Je kiest een datum en legt in je mandje, zoeken hoeft niet.', cta: 'Bekijk beschikbaarheid', from: 'vanaf', priceNote: (p, v) => `Prijzen van GetYourGuide zelf, gelezen op ${p}. Productpagina's gecontroleerd op ${v}.` },
