...
// ─────────────────────────────────────────────────────────────────────────────
// ConciergePrivacy — what happens to a private inquiry.
//
...
// real: the words `concierge` and `private@laplandvibes.com` appeared ZERO
// times in Privacy, Terms and Cookie policy combined, while
// `functions/api/concierge.ts` accepts a name, an email address, dates, party
...
// The concierge funnel is this site's own feature; the shared components cover
...
// blast radius.
//
// Keep this in step with `functions/api/concierge.ts`. If the field list or the
...
const COPY: Record<Lang, Block> = {
    ...
    heading: 'Yksityinen tiedustelu (concierge)',
  ...
  en: {
    heading: 'Private inquiry (concierge)',
  ...
  de: {
    heading: 'Private Anfrage (Concierge)',
  ...
  ja: {
    ...
    route: '送信内容は当社のサーバー（laplandluxuryvillas.com/api/concierge）を経由してSupabaseのサービス機能に渡り、private@laplandvibes.com に届きます。管理者はLapeso Oy、処理者はCloudflare（サイトとAPIのホスティング）とSupabase（メッセージ送信）です。お問い合わせの内容を第三者に販売・開示することはなく、広告のターゲティングにも使用しません。解析に渡るのは「問い合わせが送信された事実」と「その分類」のみで、本文・氏名・メールアドレスは一切渡りません。',
  ...
  es: {
    heading: 'Consulta privada (conserjería)',
  ...
  'pt-BR': {
    heading: 'Consulta privada (concierge)',
  ...
  'zh-CN': {
    ...
    route: '表单由我们自己的服务器（laplandluxuryvillas.com/api/concierge）转发至 Supabase 的服务函数，再投递到 private@laplandvibes.com。数据控制者为 Lapeso Oy，处理者为 Cloudflare（网站与接口托管）和 Supabase（消息投递）。我们不会出售或向第三方披露咨询内容，也不会将其用于广告定向。分析系统仅收到「已发送咨询」这一事实及其分类，绝不包含留言正文、姓名或邮箱。',
  ...
  ko: {
    ...
    route: '양식은 저희 서버(laplandluxuryvillas.com/api/concierge)를 거쳐 Supabase 서비스 함수로 전달되고, 메시지는 private@laplandvibes.com으로 도착합니다. 관리자는 Lapeso Oy이며, 처리자는 Cloudflare(사이트 및 API 호스팅)와 Supabase(메시지 전달)입니다. 문의 내용을 제3자에게 판매하거나 제공하지 않으며 광고 타기팅에도 사용하지 않습니다. 분석 도구에는 문의가 전송되었다는 사실과 그 분류만 전달되며, 메시지 본문과 성함, 이메일은 절대 전달되지 않습니다.',
  ...
  fr: {
    heading: 'Demande privée (conciergerie)',
  ...
  it: {
    heading: 'Richiesta privata (concierge)',
  ...
  nl: {
    heading: 'Privéaanvraag (concierge)',
  ...
  sv: {
    heading: 'Privat förfrågan (concierge)',
...
export default function ConciergePrivacy() {
  const lang = useLang()
